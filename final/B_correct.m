...
% Number of points can be 16, 21, 31, 61
delta_eta = 0.4;
eta = 0:delta_eta:6;
pt_num = length(eta);
f = (eta.^2)/12;
...
for m = 1:maxIter
    ...
    df(1) = (-3*f(1) + 4*f(2) - f(3)) / (2*delta_eta);%Right-sided OK
    dg(1) = (-3*g(1) + 4*g(2) - g(3)) / (2*delta_eta);%Right-sided
    ...
    % 邊界初始化(考慮B.C.) OK
    iter_mat(2*pt_num+1,:) = 0;
    iter_mat(2*pt_num+2,:) = 0;
    iter_mat(2*pt_num+3,:) = 0;
    [iter_mat(2*pt_num+1,1),iter_mat(2*pt_num+2,pt_num+1),iter_mat(2*pt_num+3,2*pt_num)] = deal(1);

    % Initialize the result matrix
    result_mat = zeros(2*pt_num+3,1);
    for i=1:pt_num
    ...
    if( all(abs(modf_mat(1,1:pt_num))<tol) )
        fprintf('When eta = %.3f, we need %d times of iteration\n',delta_eta, m)
        break;
    end
